...
## rules に出すか、下位 CLAUDE.md に出すか

ルート CLAUDE.md から追い出す先は 2 つある。**ルールの境界がどちらに一致するか**で決める。

- **境界がディレクトリ（パッケージ）に一致する** → そのディレクトリ直下の **CLAUDE.md**。
  「`apps/iac` では CDK context を使わない」のような、パッケージの中でだけ意味を持つルール。
- **境界がファイル型・パスで、ディレクトリ境界に一致しない横断ルール** → **path スコープ rule**。
  「全 `.ts`/`.tsx` で失敗は throw せず `Result` で返す」のように、パッケージを跨いで効くが
  ファイル型で絞れるもの（このリポの [`result-type.md`](../../../rules/result-type.md) がこれ）。

`paths` 無しの rule（無条件ロード）は、実質「ルート CLAUDE.md をファイル分割しただけ」で
コンテキスト消費は変わらない。**分割そのものを目的に使わない**——常時ロードが要るならルート
CLAUDE.md に書き、要らないなら `paths` を付けるか skill にする。
