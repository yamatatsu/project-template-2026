...
## 14. 並行リクエストとトークンリフレッシュの競合（既知の制約・未対策）

アクセストークンが失効間近だと `requireSession` が透過リフレッシュする（`middleware.ts`）。
複数の並行リクエストが同時に失効窓を踏むと**両方がリフレッシュを試みる**。リフレッシュトークンを
ローテーションするプロバイダ（Duende の既定など）では後着の refresh が `invalid_grant` になり、
§6 の分岐によりセッション破棄 → **ユーザーが突然ログアウトされる**ことがありうる。

- 本番想定の Cognito は既定で RT をローテーションしないため顕在化しない。ローカルの
  oidc-server-mock も同様（clients.json の設定次第）。
- テンプレとしては複雑度に見合わないため対策を入れず、この制約の明文化に留める。顕在化したら
  「セッション行の条件付き書き込みでリフレッシュを 1 本化」「保存済み RT と使用 RT の突合
  （後着はリトライ）」「旧 RT の短い grace 期間」のいずれかで対処する。
