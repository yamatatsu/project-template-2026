...
- **OIDC プロバイダ** = Cognito user pool + hosted UI（本番）/ oidc-server-mock（ローカル）。
...
- 単一テーブルをパーティションキー `pk` で名前空間分け:
  - `sess#<sessionId>` … トークン一式（**サーバ側のみ**）。TTL はログイン起点の絶対 30 日
    （`expiresAt`。リフレッシュの再保存で延長しない。定数は `libs/session-lifetime.ts`）。
...
- `consumeState` は Delete（`ReturnValues: ALL_OLD`）1 コマンドのワンタイム消費（state 再利用を
  防ぐ。並行 callback でも片方しか通らない）。
...
- 属性: `HttpOnly`・`Secure`（本番）・`SameSite=Strict`・`Path=/`・`Domain` 属性なし。maxAge は
  セッション本体（DynamoDB TTL）と同じ絶対 30 日（`libs/session-lifetime.ts` から導出）。
...
| | ローカル | 本番 |
| --- | --- | --- |
| OIDC プロバイダ | oidc-server-mock（`docker/oidc-server-mock/{clients,users}.json`、`:8080`） | Cognito hosted UI |
