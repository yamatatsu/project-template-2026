...
- ステータス: **実装済み**。設計の生きた記述は [`apps/backend/CLAUDE.md`](../../apps/backend/CLAUDE.md)
  「認証・認可」節へ転記済み（本ドキュメントは実装当時の計画として残す）。
- 対象: `packages/db` / `apps/backend`
- **実装時の 0000 からの変更**: `users` に `email` を**持たせない**（IdP を単一の真実の源とする。
  `session.email` は `string | undefined` で結合キーにもならず、DB に置くと二重管理になる）。これに伴い
  §5 Phase 1/3 のコード例から `email` を除き、§8 の「email 更新」未決事項を削除した。
...
export const users = pgTable(
  'users',
  {
    ...
    // email は DB に持たない（IdP / session が単一の真実の源）。
...
export const auth = (opts: { for: Access }) =>
  createMiddleware<AppEnv /* & AuthEnv */>(async (c, next) => {
    ...
    // JIT プロビジョニング: 初回アクセス時に users 行を確保する（別サインアップ導線を持たない前提）。
    // 並行初回リクエストの重複は unique(user_sub) + onConflictDoNothing で吸収し、必ず select で読み直す。
...
各保護ルートで **`auth({ for })` をハンドラの可変長 middleware 引数として渡す**。例（`routes/tasks/list.ts`）:

```ts
import { auth } from '../../middleware/auth.ts';

// 型引数は書かない。auth を middleware 引数として渡すと Hono が Env を推論し、
// c.get('user') が型安全になる。auth を外すと c.get('user') が即コンパイルエラー（書き忘れ検出）。
...
- **保護グループ配下の全ルートに `auth({ for })` を必ず付ける**（付け忘れ＝`c.get('user')` の型エラー、
  かつ authZ が効かない）。tasks はまず全て `for: 'user'`。admin 限定ルートが出たら `for: 'admin'`。
- `app.ts` の境界 `requireSession` は**そのまま**（authN は境界に残す）。`applicationRoutes` の
  `new Hono()` は無改修でよい（各ルートが Env を推論で持つため。RPC 型 `AppType` を壊さないことは
  frontend の `pnpm typecheck` で確認）。
...
1. **User プロビジョニングの方式**: 初回アクセス時 JIT 作成（`userSub` で `onConflictDoNothing` 後に
   再 select）を採用。→ 実装済み（`src/middleware/auth.ts`）。
2. **`for` の粒度**: 現状 `'user' | 'admin'` の 2 段で十分か。role を増やすなら `userRoleValues` と
   `Access` を合わせて拡張する。
3. **`applicationRoutes` の Env**: 各ルートを `new Hono<AppEnv>()` にすれば境界 `new Hono()` は無改修で
   済むことを確認済み（backend/frontend の `pnpm typecheck` が通る）。→ 解決。

> email を DB に持たせたことを前提にした「email 更新の追随」は、`users` から email を外したため未決事項
> ではなくなった（削除済み）。
