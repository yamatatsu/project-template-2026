...
（`repositories/shared/index.ts`）として値と記録の対で運び、値の決定（版の起点・`+1`・タイムスタンプ）と
...
`shared/index.ts` は repo 間で共有する永続化の実装（`Persisted`・`toPersisted`・版ヘルパ）を抱える repo
**内部**の置き場。他層（wire）が必要とする型は `repositories/types.ts` が公開面として再 export し、他層は
そこだけを import する —— 内部実装（drizzle を触る `shared`）に直接手を伸ばさせないため。
...
| `src/repositories/shared/index.ts`      | repo 共有の永続化キット（`Persisted<T>`・`toPersisted`・版ヘルパ） |
| `src/repositories/types.ts`             | 他層向けに公開する型（`Persisted` の再 export）                    |
