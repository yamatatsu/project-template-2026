...
| 404 / 409            | アクセスログの status から追える。409 は「他人の更新に負けた」だけで不正の兆候ではない                                                |
...
### 未処理エラーも requestId 付きで残る

Hono はハンドラの throw を最内周の dispatch で捕まえて `onError` を呼ぶため、`onError` は
`runInRequestScope` の**内側**で実行される（実測で確認）。合成点（`apps/backend/src/app.ts`）の
`onError` が `getLogger().error('unhandled error', err)` で構造化ログに残し、レスポンスは内訳を
漏らさない 500 JSON に固定する。Hono 既定の `onError`（`console.error`）に任せると、スタックが
requestId 無しで構造化ログの外に出てしまう。なおこの場合もアクセスログは status=500 で 1 行出る
（`onError` がレスポンス化するので、requestLogger の `next()` は reject しない）。
