...
- **`src/entities/<feature>.ts`（純粋）** —— エンティティ型・factory（`createTask`）・状態遷移
  （`applyUpdate`。遷移ルールはここに書き、ルートに散らさない）。DB もフレームワークも触らない。
  **記録メタデータ（`version`・`createdAt`/`updatedAt`）を持たせない**（永続化の関心。
  [`docs/specs/optimistic-lock.md`](../../docs/specs/optimistic-lock.md)）。
  - 失敗しうる関数は throw せず `Result` で返す
    （[`.claude/rules/result-type.md`](../../.claude/rules/result-type.md)）。`E` はタグ付き union。
  - **ドメインで `new Date()` しない**。時間を読む業務ルールが要るときは `now` を引数で注入する
    （「クライアントの意図」（`updates` 等）とは別引数にする）。
- **`src/repositories/<feature>-db-repo.ts`（永続化）** —— DB の読み書きだけ。業務判断を持たない。API は
  **`find`/`add`/`save`/`remove` のコレクション語彙**でそろえ、SQL/CRUD 動詞は使わない。`find*` は nullable
  （`get*` にしない）。**記録メタデータは repo が所有する**: `Persisted<T>`（`repositories/shared/index.ts`）で
  値と対にして返し、版の起点・増分・タイムスタンプ・CAS を repo に閉じる。DB 行 → 対の変換は共有ヘルパ
  `toPersisted`、版の増分は共有 SQL 片 `incrementVersion` を使う。**他層が使う型は `repositories/types.ts`
  から取り**、内部の `shared` を直接 import しない。
- **ルート（アプリケーション層）** —— `findTask` → `applyUpdate` → `saveTask` を繋ぎ、HTTP と結果コードの
  対応づけだけ持つ。**ワイヤ⇄ドメインの組み替え**（フラットな入力を `updates` と `expectedVersion` に
  分ける等）はここに置き、ドメインにワイヤの語彙を持ち込まない。
- **列の値は DB のデフォルトではなくアプリが決める**（[`packages/db/CLAUDE.md`](../../packages/db/CLAUDE.md)）。
  業務値はドメイン（`createTask`／`applyUpdate`）、記録メタデータは repo（`addTask`／`saveTask`）が決める。
  **`$onUpdate` は使わず、ハンドラでもセットしない**。
- **DELETE はドメイン関数を挟まない**（状態遷移が無い）。repo の `removeTask` を直接呼ぶ。
...
- **`ETag` / `If-Match` は使わない**（HTTP の条件付きリクエストを実装しない）。版はリソースの属性として
  ...
  **版を受けるミドルウェアも作らない** —— 検証は PUT の `zValidator('json', conditionalTaskInputSchema)` が
  兼ねる。
- **版の一致判定は `saveTask` の CAS 一本**（`WHERE version = expectedVersion`。増分は共有 SQL 片
  `incrementVersion`（DB 側の `version + 1`）を repo が書く）。メモリ内の事前チェックは置かない（CAS の
  部分集合）。ドメインは版を知らない。
- **クライアントが編集の土台にした版は `expectedVersion` と呼ぶ**（wire・repo の CAS 引数とも。
  `currentVersion` は「今 DB にある版」と読み違える）。
