...
マウントし、機能ルート（`/me`・`/tasks`）を保護グループに束ねてルート直下に合成する。保護グループは
境界で `auth.requireSession` を 1 回適用して配下を認証保護する。`/me` は permissions（role 由来の app
固有 RBAC）を返すため backend-auth ではなく apps/backend が所有し、この保護グループに同居する
（「認証・認可」）。設定は `AppConfig` として注入し、env 直読みはアプリ内でしない（検証は起動時に一括）。
...
- **テストもルート単位で分割**（`tasks.list.test.ts` 等、1 ルート 1 ファイル・同じフラット命名）。各
  テストは対象の default export を `withSession` で包み、`hono/testing` の `testClient` の**型付き RPC
  クライアント**で叩く（`client.tasks[':id'].$put({ param, json, header })`）。リクエストの param/json/header
  が RPC の型で検証されるので、テストのボディがサーバ契約とずれれば型で落ちる（body 型は
  `InferRequestType` で route から取り出す）。契約違反を意図的に送る negative test（必須欠落・If-Match
  欠如）だけ `as never` で型を外す。1 ファイルに集約しない（肥大化する）。
...
- **authZ は各ルートのハンドラ定義に同居**。`auth({ action })`（`src/middleware/auth.ts`）を**ハンドラの
  ...
  要求 action が user の permissions に無ければ **403**（未認証 401 は境界の requireSession が担う分担）。
  action を省略した `auth()` は role を解決するだけで permission を要求しない（`/me` が使う）。この authZ は
  `db`（静的 import）と Context 参照だけで成立し設定注入が不要なので静的 import で直接書ける。要求 action を
...
- **レイヤリング**: `@icasu/backend-auth` は session / identity（`userSub`・`email`）まで。再利用パッケージを
  ...
  `c.get('user')` だけを見る。**`/me` も同じ理由で apps/backend が所有**する（permissions は role 由来の app
  固有情報なので backend-auth には置けない。`routes/me.get.ts` が `{ userSub, email, permissions }` を返す）。
...
- **`users` に `email` は持たせない**。email は IdP（Cognito → session）が単一の真実源で、DB に置くと二重
  ...
  判断はアプリ層（`auth` + `authorization.ts`）に置きドメインに持ち込まない。
