...
describe('TasksOverview', () => {
  it('renders a read-only row per task with badges and no link', async () => {
    ...
    await screen.findByTestId('tasks-overview');

    // トップは読み取り専用なのでタイトルはリンクにしない。
    expect(screen.queryByRole('link', { name: 'タスクA' })).not.toBeInTheDocument();
  ...
  it('shows the empty state for an empty list', async () => {
    ...
    expect(await screen.findByTestId('tasks-overview-empty')).toBeInTheDocument();
    expect(screen.queryByTestId('tasks-overview')).not.toBeInTheDocument();
  });
