...
- `route.test.ts` — `createAuthRoute` の Hono app を実 Cookie で駆動し、login → callback →
  logout の BFF フロー全体（PKCE・state・Cookie 署名・自動リフレッシュ・オープンリダイレクト防止）を
  記述する。`/me` 本体はホストが所有するため、ここでは `requireSession` を差した最小の保護プローブ
  （`/probe`）で Cookie 検証・自動リフレッシュ・ログアウト後の失効という統合挙動だけを見る。注入する
  フェイクは2つの境界のみ: セッションストア（in-memory Map）と、トークンエンドポイント / id_token 検証
  （`exchangeCode`/`refreshTokens`/`verifyIdToken` の `vi.fn()`）。
